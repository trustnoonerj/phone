...
QBCore.Functions.CreateCallback('qb-phone:server:GetGarageVehicles', function(source, cb)
    ...
    for _, v in pairs(result) do
        ...
        local modifications = json.decode(v.mods or '{}') -- safe decode
        local fuel = modifications.fuelLevel or 100
        local engine = modifications.engineHealth or 1000
        local body = modifications.bodyHealth or 1000

        -- Garage name
        local garageLabel = "Unknown"
        local garageKey = v.garage

        if Config.Garages[garageKey] then
        ...
        local stateLabel = "In"
        if v.state == 0 then
            stateLabel = v.depotprice == 0 and "Out" or "In Depot"
            garageLabel = v.depotprice == 0 and "None" or "Depot"
        elseif v.state == 2 then
            stateLabel = "Impounded"
            garageLabel = "Police Depot"
        end

        local vehdata = {
            fullname = vehicleData and (vehicleData.brand .. " " .. vehicleData.name) or v.vehicle,
            brand = vehicleData and vehicleData.brand or v.vehicle,
            model = vehicleData and vehicleData.name or v.vehicle,
            plate = v.plate,
            garage = garageLabel,
            state = stateLabel,
            fuel = fuel,
